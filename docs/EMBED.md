...
5. Confirm:
   ...
   - Land on `/confirmation` inside the iframe.
...
- BNPL "break-out" mode: launch the Stripe redirect via `window.top.location` so the user leaves the embed, completes BNPL on our full domain, and lands on a non-embedded `/confirmation`.
