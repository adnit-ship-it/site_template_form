...
│                               # (Product/category catalog is DB-backed —
│                               #  see composables/useProductsCatalog.ts +
...
pages/checkout.vue (layout: checkout)
        ...
        └─ Step 2: CheckoutPaymentStep
            ...
            └─ Submit:
                ...
                └─ navigateTo('/confirmation?confirmation=...')
```
...
### Feature Flags
Runtime feature flags live in Supabase (`site_config` table) rather than env
vars — see [CONFIG_SPLIT.md](./CONFIG_SPLIT.md). Read them via the
`useRuntimeConfigDB()` composable:

- `siteConfig.useNewAPI` — switches between `buildFormPayload` (old) and `buildFormPayloadNew` (new), and changes the submission endpoint from `/dynamic-case` to `/cases`.
- `siteConfig.bnplActivated` — enables/disables Buy Now Pay Later payment options. Composed with `useIsEmbedded()` at the consumer because BNPL also auto-disables inside iframes.
- `siteConfig.homeUrl` — confirmation page "Go home" CTA target.
- `siteConfig.everflowBaseUrl` / `everflowApiKey` / `everflowOfferId` / `everflowEventId` — for the (currently dormant) Everflow integration.
- `siteConfig.defaultQuizIdOverride` — optional runtime override of `data/common.json:quiz.defaultQuizId`.

Defaults defined in [`types/runtime-config.ts`](../types/runtime-config.ts) cover the case where Supabase is unreachable or the org row doesn't exist yet.
...
| `#payment-success` | Confirmation card on confirmation page | `pages/confirmation.vue` |
