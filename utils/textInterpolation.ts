...
function getOrgName(): string {
  try {
    const fromBranding = useRuntimeConfigDB().value.branding?.brand?.orgName?.trim();
    if (fromBranding) return fromBranding;
    const fromPublic = useRuntimeConfig().public.orgName as string | undefined;
    if (fromPublic?.trim()) return fromPublic.trim();
  } catch {
    // outside Nuxt context (tests)
  }
