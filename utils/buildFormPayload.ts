...
import { getQuizById, getDefaultQuizId } from "~/data/quizConfigs";
...
export interface PaymentInfo {
  ...
  // the Stripe SetupIntent/PaymentIntent reference on the wire payload.
  nmiPaymentToken?: string;
  paymentDate?: string;
  validUntil?: string;
  status?: string;
  promoCode?: string;
  // Subscription plan key chosen by the user. Only consumed by the new API
  // builder (`buildFormPayloadNew`), where it maps to the `subscription`
  // block on each product. Ignored by this legacy builder.
  plan?: string;
}
...
export async function buildFormPayload(
  ...
  // NMI path: a Collect.js token replaces the Stripe SetupIntent/PaymentIntent.
  // When present we skip Stripe id resolution/validation entirely.
  const nmiPaymentToken = paymentInfo?.nmiPaymentToken;

  let finalStripeId: string | undefined;
  if (!nmiPaymentToken) {
    finalStripeId = resolveStripeId(formAnswers, paymentInfo);
  ...
  const payload: any = {
    ...
    shippingAddress,
    formTitle: (await getQuizById(quizId || getDefaultQuizId()))?.name || 'Medical Intake Form',
    formDescription: (await getQuizById(quizId || getDefaultQuizId()))?.description || 'Patient medical intake form',
  ...
  // Attach the payment reference: NMI token (top-level) or the Stripe id.
  if (nmiPaymentToken) {
    payload.nmiPaymentToken = nmiPaymentToken;
  } else if (finalStripeId && finalStripeId !== "ready") {
