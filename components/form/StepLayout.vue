<template>
  <div ref="stepContainer" :id="`question-${stepData.id}`" class="w-full max-w-[668px] mx-auto">
...
// Read `selectedQuizId` directly from the route instead of calling
// `usePatientForm()`. Calling the composable here has a serious side effect:
// it runs `loadAndInitQuiz(..., true)`, which in a `nextTick` calls
// `restoreClientState` and snaps `currentStepIndex` back to the furthest
// completed step. Because this component remounts on every step change
// (thanks to the fade Transition's `:key`), that snap-back was causing the
// "Back button flashes the previous step then returns to the current one" bug.
// Mirrors the logic in `usePatientForm`'s own `selectedQuizId` computed.
