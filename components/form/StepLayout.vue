...
// Safe to call here post-singleton-refactor: `usePatientForm()` now joins
// the shared per-quiz registry (Nuxt `useState`) instead of re-initialising
// state on every mount, so calling it from a component that remounts on
// every step change no longer snaps `currentStepIndex` back to the
// last-completed step.
