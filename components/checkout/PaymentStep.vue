  ...
  <div class="relative">
    ...
    <div class="max-w-[726px] mx-auto mb-8 lg:-mt-10">
      <div class="bg-[#01cd8b]/10 rounded-2xl lg:rounded-[28px] p-4 lg:py-8 lg:px-12">
        ...
        <div v-if="selectedProduct" class="flex gap-2 md:flex-row flex-col items-center lg:gap-4 md:gap-6 mb-6">
          ...
          <div class="flex-1 bg-[#75af9c]/30 rounded-[28px] p-4 md:p-6 w-full md:w-auto">
            <div class="flex flex-col gap-2 text-[13px] lg:text-base">
              ...
              <div v-if="Number(formatTotalSavings()) > 0" class="flex justify-between items-center">
        ...
        <!-- Savings Text — hidden when there's nothing to save (e.g. monthly plan with no promo) -->
        <p v-if="Number(formatTotalSavings()) > 0" class="text-center text-sm md:text-base text-[#4a5565] mb-4">
        ...
        <!-- Promo Code Banner -->
        <div v-if="!useNewAPI" ref="promoCodeBannerRef" @click="promoInputRef?.focus()"
    ...
    <div class="max-w-[726px] mx-auto">
      ...
      <div class="mb-6">
        ...
        <div class="bg-white border rounded-[8px] border-[#d9d9d9] p-4 md:p-6">
          ...
          <div v-else class="w-full space-y-4 animate-pulse">
            <div>
      ...
      <div class="mb-6">
        ...
        <div class="bg-white border rounded-[8px] border-[#d9d9d9] p-4 md:p-6">
          <!-- Stripe card / BNPL path (useStripe = true) -->
          <template v-if="useStripe">
            ...
            <div v-if="!isStripeLoaded && (paymentMethodType === 'card' || paymentMethodType === 'bnpl')"
              class="w-full space-y-4 animate-pulse">
              <div>
          ...
          <!-- NMI card path (useStripe = false). Card details are collected
               via NMI Collect.js; the Stripe Address Element above still
               handles shipping. BNPL is hidden in this mode. -->
          <div v-else class="w-full">
            <NmiPaymentOnly ref="nmiPaymentRef" :form-answers="formAnswers"
              @payment-update="handlePaymentUpdate" />
          </div>
        </div>
      </div>
      ...
      <div class="mb-6">
        <label class="flex items-start gap-3 cursor-pointer">
          ...
          <span class="text-sm text-bodyColor/70" v-html="consentTextHtml"></span>
        </label>
      </div>

      <!-- Submit Button -->
      <button id="checkout-submit" @click="handleSubmit" :disabled="!canSubmit || props.isSubmitting || isSubmittingLocal"
...
import NmiPaymentOnly from '~/components/checkout/NmiPaymentOnly.vue'
...
// `useNewAPI` and `bnplActivated` moved to Supabase (`site_config` table).
// They're now read reactively via the runtime-config-db plugin / composable;
// template auto-unwrap means consumers reference them by name like before.
const runtimeDB = useRuntimeConfigDB()
const useNewAPI = computed(() => runtimeDB.value.siteConfig.useNewAPI)

// Card payment provider. When false, card details are collected via NMI
// Collect.js instead of Stripe Elements (the Stripe Address Element is still
// used for shipping). See `site_config.use_stripe` and docs/SUPABASE_SCHEMA.md.
const useStripe = computed(() => runtimeDB.value.siteConfig.useStripe)

// BNPL is disabled when the app is running inside an iframe. BNPL triggers a
// full-page redirect to Stripe → Klarna/Affirm and back to /checkout — that
// redirect happens inside the iframe only, and cross-origin storage
// partitioning makes the return trip unreliable. Card payments stay inside
// the iframe and work without any of that. See docs/EMBED.md.
// BNPL also requires Stripe, so it's force-hidden whenever `useStripe` is off.
const isEmbedded = useIsEmbedded()
const bnplActivated = computed(
  () => runtimeDB.value.siteConfig.bnplActivated && !isEmbedded.value && useStripe.value
)

const stripeAddressRef = ref()
const stripePaymentRef = ref()
const stripeBNPLRef = ref()
const nmiPaymentRef = ref()
...
// Consent text — kept as plain text because it's also emitted upstream as
// the actual stored/audited consent value (see `update-consent` emit below).
// The display version with hyperlinks lives in `consentTextHtml` and is the
// one rendered via `v-html`.
...
// Display-only version with hyperlinks. `onclick="event.stopPropagation()"`
// keeps clicks on the links from bubbling to the wrapping <label> and
// toggling the consent checkbox. `target="_blank"` so the user doesn't lose
// their checkout state.
const consentTextHtml = consentText.replace(
  "Terms of Service and Privacy Policy",
  '<a href="/terms" target="_blank" rel="noopener noreferrer" onclick="event.stopPropagation()" class="underline text-accentColor1 hover:opacity-80">Terms of Service</a> and <a href="/privacy" target="_blank" rel="noopener noreferrer" onclick="event.stopPropagation()" class="underline text-accentColor1 hover:opacity-80">Privacy Policy</a>'
)
...
// Dev-only debug: logs the moment any of the five Continue-gating flags
// change, so it's obvious which one is keeping `canSubmit` false. Strip
// or comment out once the wiring is confirmed working.
if (import.meta.dev) {
  watch(
    [
      isAddressComplete,
    ...
    () => {
      console.log('[PaymentStep canSubmit debug]', {
        paymentMethodType: paymentMethodType.value,
        isAddressComplete: isAddressComplete.value,
        isPaymentComplete: isPaymentComplete.value,
        isBNPLComplete: isBNPLComplete.value,
        shippingAddressIsNonNull: shippingAddress.value !== null,
        consentChecked: consentChecked.value,
