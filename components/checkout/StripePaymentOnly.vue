...
// Runtime config is read at script-setup scope so the test-card guard
// inside `paymentElement.on('change', ...)` (which fires every keystroke
// once the form is filled) can read it without throwing. Earlier this
// referenced an undeclared `config` variable, which threw a ReferenceError
// the moment the card form became `complete: true` — that error blocked
// the subsequent `emit('payment-update', isComplete)` call, so the parent
// PaymentStep never saw the card flip to complete and the Continue button
// stayed disabled.
...
onMounted(async () => {
  ...
  try {
    ...
    paymentElement.on('change', (e: any) => {
      ...
      // Test card detection — only enforced in production builds. The
      // runtime config exposes `isProduction` (boolean) from
      // nuxt.config.ts; there is no `nodeEnv` field on `public`.
      if (isComplete && runtimeConfig.public.isProduction) {
        try {
