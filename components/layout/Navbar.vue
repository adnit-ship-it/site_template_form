<template>
  <!-- AnnouncementBar lives with the nav chrome. Vue 3 fragments let it sit
       as a sibling of <nav> without a wrapper div. The bar is fixed-position
       at top:0 and the nav offsets via the --site-announcement-height CSS
       variable set in app.vue, so DOM nesting doesn't change layout. -->
  ...
  >
    ...
    <NuxtLink v-else-if="!hideTopBar && showMarketing" to="/consultation" :class="['w-full h-[44px] lg:h-[52px] flex items-center justify-center bg-[#E4ECF7] cursor-pointer hover:bg-[#E4ECF7]/90 transition-colors overflow-hidden']">
    ...
    <div :class="[
      'relative w-full flex items-center py-2 px-4 max-w-[1304px] mx-auto',
      (centerLogo || hidebtn || logoOnly) ? 'justify-center' : 'justify-between',
    ]">
      <!-- Back button slot: any page can call `useNavbarBack().setBack(handler)` to make this appear.
           Wrapped in <ClientOnly> because:
             1. It's a pure client-side interaction (no meaning on SSR).
             2. `useNavbarBack` keeps its handler in a module-scoped ref (functions
                aren't SSR-payload-serializable). That ref starts null on the client
                but can be set during SSR by a page's `{ immediate: true }` watch —
                the resulting "button on server, comment on client" mismatch shows up
                as a Hydration node mismatch warning at <LayoutNavbar>. Skipping SSR
                for the button entirely sidesteps that. -->
      <ClientOnly>
        <button
          v-if="backHandler"
          type="button"
          ...
          class="absolute left-3 lg:left-4 top-1/2 -translate-y-1/2 z-10 flex items-center justify-center w-9 h-9 lg:w-11 lg:h-11 rounded-full bg-white hover:bg-gray-50 active:scale-95 transition-all shadow-sm border border-gray-200"
        >
          <svg
            xmlns="http://www.w3.org/2000/svg"
            class="h-5 w-5 lg:h-6 lg:w-6 text-bodyColor"
            fill="none"
            viewBox="0 0 24 24"
            stroke="currentColor"
          >
      ...
      <div class="flex items-center gap-2 lg:gap-3">
        <NuxtLink to="/consultation">
          <img src="/assets/images/brand/logo.png" alt="Brand Logo" class="h-[32px] lg:h-[64px] w-auto" />
        </NuxtLink>
      ...
      <div v-if="!logoOnly" class="flex items-center gap-4">
        <UiButton :class="'!font-bold text-nowrap text-xs min-[380px]:text-[13px]'" v-if="showMarketing && !hidebtn" to="/consultation" color="accentColor2" label="take the quiz" :showArrow="true" arrowColor="#000000"
    ...
    <div v-if="!logoOnly && isMobileMenuOpen"
      ...
      <div class="max-w-5xl mx-auto px-4 py-4">
        <NuxtLink to="/consultation" @click="closeMobileMenu" class="block py-3 px-4 hover:bg-gray-50 rounded transition-colors">
          Home
        </NuxtLink>
        <NuxtLink to="/confirmation" @click="closeMobileMenu" class="block py-3 px-4 hover:bg-gray-50 rounded transition-colors">
          Confirmation
        </NuxtLink>
...
// Back button slot (populated by pages via `useNavbarBack().setBack(...)`)
const { backHandler, backLabel, triggerBack } = useNavbarBack();
