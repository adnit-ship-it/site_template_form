...
/**
 * Fallback default quiz id when Supabase is unavailable (from
 * `DEFAULT_BRANDING`). Most consumers should call `getDefaultQuizId()`.
 */
export const FALLBACK_DEFAULT_QUIZ_ID: string =
  DEFAULT_BRANDING.quiz?.defaultQuizId?.trim() || "weight-loss";

/** @deprecated Use `FALLBACK_DEFAULT_QUIZ_ID` */
export const REPO_DEFAULT_QUIZ_ID = FALLBACK_DEFAULT_QUIZ_ID;

/**
 * Resolves the default quiz id used everywhere a fallback is needed.
 *
 * Resolution order:
 *   1. `site_config.default_quiz_id_override` (hot-swap)
 *   2. `site_branding.default_quiz_id` (via runtime `branding.quiz`)
 *   3. `FALLBACK_DEFAULT_QUIZ_ID`
 */
export function getDefaultQuizId(): string {
  const runtimeDB = useRuntimeConfigDB();
  const override = runtimeDB.value.siteConfig.defaultQuizIdOverride?.trim();
  if (override) return override;
  const fromBranding = runtimeDB.value.branding?.quiz?.defaultQuizId?.trim();
  return fromBranding || FALLBACK_DEFAULT_QUIZ_ID;
}
...
const quizLoaders: Record<string, () => Promise<QuizConfig>> = {
  "weight-loss": async () => {
    ...
    return {
      ...
      stepProgressMapping: [
        ...
        { stepId: "medicalConditions3", progressStepId: "health" },
  ...
  "skin-care": async () => {
    const { skinSteps, contactFormSteps } = await import("./formSteps");
    return {
      id: "skin-care",
      name: "Skin Care Intake Form",
      description:
        "Medical intake form for prescription skincare. Please provide complete and truthful information to help your physician determine the best treatment for you.",
      version: "1.0.0",
      progressSteps: [
        { id: "basics", name: "Basics", description: "Body metrics and skincare goals" },
        { id: "health", name: "Health", description: "Skin conditions, medications, and allergies" },
        { id: "details", name: "Details", description: "Current routine and additional info" },
      ],
      stepProgressMapping: [
        { stepId: "heightWeight", progressStepId: "basics" },
        { stepId: "gender", progressStepId: "basics" },
        { stepId: "skinPriorityGoal", progressStepId: "basics" },
        { stepId: "skinOtherGoals", progressStepId: "basics" },
        { stepId: "skinDryness", progressStepId: "basics" },
        { stepId: "skinSensitivity", progressStepId: "basics" },
        { stepId: "usingTretinoin", progressStepId: "health" },
        { stepId: "tretinoinStrength", progressStepId: "health" },
        { stepId: "skinConditions", progressStepId: "health" },
        { stepId: "skinCurrentConditions", progressStepId: "health" },
        { stepId: "skinTopicalAllergies", progressStepId: "health" },
        { stepId: "skinAllergies", progressStepId: "health" },
        { stepId: "skinMedications", progressStepId: "health" },
        { stepId: "skincareUsage", progressStepId: "details" },
        { stepId: "skinAdditionalInfo", progressStepId: "details" },
        { stepId: "dob", progressStepId: "details" },
        { stepId: "personalInfo", progressStepId: "details" },
      ],
      steps: [...skinSteps, ...contactFormSteps],
    };
  },

  "hair-loss": async () => {
    const { hairSteps, contactFormSteps } = await import("./formSteps");
    return {
      id: "hair-loss",
      name: "Hair Loss Intake Form",
      description:
        "Medical intake form for prescription hair loss treatment. Please provide complete and truthful information to help your physician determine the best treatment for you.",
      version: "1.0.0",
      progressSteps: [
        { id: "goals", name: "Goals", description: "Hair satisfaction, treatment goals, and location" },
        { id: "history", name: "History", description: "Past treatments and scalp symptoms" },
        { id: "health", name: "Health", description: "Medical conditions, medications, and allergies" },
      ],
      stepProgressMapping: [
        { stepId: "heightWeight", progressStepId: "goals" },
        { stepId: "gender", progressStepId: "goals" },
        { stepId: "hairSatisfaction", progressStepId: "goals" },
        { stepId: "hairLossGoals", progressStepId: "goals" },
        { stepId: "hairLossLocation", progressStepId: "goals" },
        { stepId: "hairTreatmentHistory", progressStepId: "history" },
        { stepId: "hairTreatmentReceived", progressStepId: "history" },
        { stepId: "hairTreatmentExperience", progressStepId: "history" },
        { stepId: "hairScalpSymptoms", progressStepId: "history" },
        { stepId: "hairBloodPressureHistory", progressStepId: "health" },
        { stepId: "hairMedicalConditions", progressStepId: "health" },
        { stepId: "hairCurrentMedications", progressStepId: "health" },
        { stepId: "hairTopicalAllergies", progressStepId: "health" },
        { stepId: "hairAllergies", progressStepId: "health" },
        { stepId: "hairAdditionalInfoYesNo", progressStepId: "health" },
        { stepId: "hairProviderMessage", progressStepId: "health" },
        { stepId: "dob", progressStepId: "health" },
        { stepId: "personalInfo", progressStepId: "health" },
      ],
      steps: [...hairSteps, ...contactFormSteps],
    };
  },
