<template>
  ...
  <div ref="checkoutContainerRef" class="container mx-auto px-4 md:p-6 lg:p-8 pb-24 md:pb-32">
    <div class="max-w-[1248px] mx-auto">
      ...
      <!-- Step 2: Payment -->
      <CheckoutPaymentStep id="checkout" ref="paymentStepRef" v-if="currentStep === 2"
...
import { getVariationPrice, getVariationId, getVariationBundleId } from '~/types/checkout'
...
// Products catalog (DB-backed) — populated by `plugins/02.products-catalog.ts`
// at app startup. Both `categories` and `products` are reactive refs
// (`ComputedRef<Category[]>` / `ComputedRef<Product[]>`); use `.value` when
// reading imperatively from script.
const { categories, products } = useProductsCatalog()
...
// When embedded in an iframe, BNPL is disabled (card-only). The UI in
// PaymentStep.vue hides the BNPL toggle based on this, but we also guard
// the payment processing path below as a belt-and-suspenders defense.
//
// `useStaging` mirrors the server-side env decision (`resolveEnvMode`): dev
// builds and embedded sessions submit to the CareValidate staging API, so we
// resolve each variation's staging id/bundleId (when present) before building
// the case payload.
const { isEmbedded, useStaging } = useEnvMode()

// Card payment provider flag (Supabase `site_config.use_stripe`). When false,
// the card is tokenized via NMI Collect.js instead of Stripe, and BNPL is
// hidden. Named `useStripeFlag` to avoid colliding with the `useStripe`
// composable imported above.
const useStripeFlag = computed(() => useRuntimeConfigDB().value.siteConfig.useStripe)
...
// Wire the navbar's back button to the appropriate action for the current step.
// Placed after `goBackToQuiz` is defined so the `immediate` watch can safely
...
watch(
  ...
  (step) => {
    if (step === 2) {
      setBack(() => prevStep(), 'Back to treatment selection')
    } else {
      setBack(() => goBackToQuiz(), 'Back to quiz')
    }
  },
  { immediate: true }
)
...
onMounted(async () => {
  ...
  if (productId) {
    ...
    if (matchingProducts.length > 0) {
      ...
      foundCategory = categories.value.find(c => c.id === categoryId) || null
    }
  } else {
...
watch(currentStep, async (newStep, oldStep) => {
  ...
  // Fire GTM events when payment step is shown
  if (newStep === 2 && oldStep !== 2) {
    pushDataLayer({ event: 'PAYMENT_LOAD' });
    pushDataLayer({
      event: 'PRODUCT_SELECTED',
      product_name: selectedProduct.value?.name || null,
      product_plan: selectedPlan.value || 'monthly',
    });
...
const storeCheckoutDataForBNPL = async () => {
  ...
  const quizId = selectedProduct.value?.quizId || categoryId_bnpl || getDefaultQuizId()
  ...
  const getProductBundleId = () => {
    ...
    const plan = selectedPlan.value || 'monthly'
    return getVariationId(selectedProduct.value, plan, useStaging.value) || ''
  }

  const getLegacyBundleId = () => {
    if (!selectedProduct.value) return ''
    const plan = selectedPlan.value || 'monthly'
    return getVariationBundleId(selectedProduct.value, plan, useStaging.value) || ''
  }

  const finalProductBundleId = getProductBundleId()
  const finalLegacyBundleId = getLegacyBundleId()
  ...
  // Store simple product data. `legacyBundleId` is persisted alongside the
  // variation `productBundleId` so the BNPL restore path can forward both to
  // submitPatientForm and route them to the right API builder.
  const productData = {
    paymentDescription,
    paymentAmount,
    productBundleId: finalProductBundleId,
    legacyBundleId: finalLegacyBundleId,
    plan: selectedPlan.value || 'monthly',
  }
  localStorage.setItem('bnpl_product_data', JSON.stringify(productData))

  // Store full checkout data. `plan` is forwarded so the BNPL restore path
  // can rebuild a correct `subscription` block on the new-API payload.
  const paymentInfo = {
    paymentDescription,
    paymentAmount,
...
const handleBNPLPaymentSuccess = async (paymentIntentId: string) => {
  try {
    ...
    const storedQuizId = checkoutData.quizId || getDefaultQuizId()
    ...
    const variationLegacyBundleId = productData?.legacyBundleId || ''
    ...
    // Prepare payment info with BNPL paymentIntentId. `plan` is preserved
    // from `storedPaymentInfo` (which spreads first) so `buildFormPayloadNew`
    // sees a correct `subscription` block; we also fall back to the plan
    // persisted on `bnpl_product_data` for older sessions.
    const bnplPaymentInfo = {
      ...
      plan: storedPaymentInfo?.plan || productData?.plan || 'monthly',
    }
...
const processPaymentWithConfirmation = async () => {
  ...
  // When embedded, BNPL is force-disabled regardless of any state the child
  // component might still hold — the redirect flow would break inside an
  // iframe. See docs/EMBED.md.
  const isBNPLPayment =
    !isEmbedded.value && (paymentData.value as any)?.paymentType === 'bnpl'
  ...
  try {
    ...
    // Confirm the card and obtain a payment reference. Stripe path resolves a
    // SetupIntent id via window events from StripePaymentOnly; NMI path calls
    // the NmiPaymentOnly component directly and resolves a Collect.js token.
    let confirmedSetupIntentId: string | undefined
    let confirmedNmiToken: string | undefined
    try {
      if (useStripeFlag.value) {
        ...
        // NMI: tokenize the card via Collect.js (NmiPaymentOnly.performPayment)
        confirmedNmiToken = await paymentStepRef.value?.nmiPaymentRef?.performPayment()
        if (!confirmedNmiToken) {
          throw new Error('Card tokenization failed. Please re-enter your card details.')
        }
      }

      toast.success('Payment confirmed successfully!')

      // Payment method confirmed — fire sign_up event just before submitting the case
      ...
      // `plan` is consumed by `buildFormPayloadNew` to set the
      // `products[0].subscription` block on the new-API payload.
      const paymentInfo = {
        ...
        nmiPaymentToken: confirmedNmiToken,
        promoCode: paymentStepRef.value?.promoCode || undefined,
        plan: selectedPlan.value || 'monthly',
      }

      // Get productBundleId (variation.id, used by the new API + tracking)
      // and the legacy bundleId (variation.bundleId, used by the legacy API
      // payload's `productBundleId` field) from the selected variation.
      const selectedProductBundleId = selectedProduct.value
        ? (getVariationId(selectedProduct.value, selectedPlan.value || 'monthly', useStaging.value) || '')
        : ''
      const selectedLegacyBundleId = selectedProduct.value
        ? (getVariationBundleId(selectedProduct.value, selectedPlan.value || 'monthly', useStaging.value) || '')
        : ''
      ...
      await submitPatientForm(
        ...
        selectedProductBundleId, // variation.id → new API + tracking
        selectedLegacyBundleId   // variation.bundleId → legacy API field
      )
    ...
    if (isCardDeclined) {
      ...
      // Remount only the payment element (preserves address) with a fresh
      // SetupIntent. Stripe-only — NMI fields persist and can be retried.
      if (useStripeFlag.value) {
      ...
      // Stripe-only recovery: reset payment state + remount the Stripe payment
      // element with a fresh SetupIntent. For NMI the entered card fields stay
      // valid so the user can simply retry Continue.
      if (useStripeFlag.value) {
        resetPaymentState()
        paymentStepRef.value?.remountPaymentElement()
      }
