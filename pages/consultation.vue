<template>
  <div class="flex flex-col items-center relative min-w-full px-4 lg:px-0">
    ...
    <!-- DISQUALIFIED SCREEN -->
    <FormDisqualifiedScreen
      v-if="isDisqualified"
      :title="disqualifyMessage?.title"
      :subtitle="disqualifyMessage?.subtitle"
      :show-back-button="disqualifyMessage?.showBackButton !== false && !!disqualifyMessage?.title"
      @back="backFromDisqualified"
      @shop-more="handleShopMore"
    />
...
// `resolve-product-quiz` runs the `?productId=<slug>` -> Supabase
// `quizId` lookup BEFORE this page's setup runs and stashes the result
// in `useState('resolvedProductQuizId')`, which `usePatientForm()` reads
// synchronously. Doing the await in middleware (instead of inline here)
// keeps `<script setup>` synchronous, which is required to avoid the
// "Hydration node mismatch" warning Nuxt's <Suspense> emits when an
// async page setup is hydrated against an SSR-rendered fragment.
definePageMeta({
  layout: "consultation",
  middleware: ["resolve-product-quiz"],
});
...
// Clear localStorage and navigate when user clicks "Shop more" / homepage CTA
const handleShopMore = (url: string) => {
  if (process.client) {
    try {
      localStorage.removeItem(`quiz_${selectedQuizId.value}_data`);
      localStorage.removeItem(`quiz_${selectedQuizId.value}_completed`);
    } catch {}
...
watchEffect(() => {
  if (isDisqualified.value) {
    // Per-step disqualifications can be reversed by going back and changing
    // the offending answer (provided the message hasn't opted out of it).
    if (disqualifyMessage.value?.showBackButton !== false && !!disqualifyMessage.value?.title) {
      setBack(() => backFromDisqualified(), "Back to quiz");
    } else {
      clearBack();
    }
...
const checkQuizCompletionAndRedirect = async (): Promise<boolean> => {
  ...
  let quizId = categoryId || selectedQuizId.value || getDefaultQuizId();
...
onMounted(async () => {
  ...
  everflow.beginQuiz();
  // Note: `productSelected` used to fire here too, but per the Everflow
  // event spec it belongs on the checkout step-1 → step-2 transition.
  // It's now fired from `useCheckout().nextStep()` when the user advances
  // past the variation selector.
});
</script>
