<template>
  <div class="flex flex-col items-center relative min-w-full px-4 lg:px-0">
    ...
    <!-- NORMAL FORM FLOW -->
    <template v-else>
      <!-- Top Navigation Bar with Progress Tracker -->
      <div class="w-full max-w-[768px] mx-auto mb-0">
        <button @click="prevStep" :disabled="currentQuestionIndex === 0 || isLoading"
          class="absolute -top-16 left-4 z-[999999] lg:hidden bg-white rounded-full p-2 text-bodyColor disabled:opacity-30 disabled:cursor-not-allowed transition-opacity shadow-sm border border-gray-200">
        ...
        <div class="w-full flex items-center gap-4">
          ...
          <button @click="prevStep" :disabled="currentQuestionIndex === 0 || isLoading"
            class="hidden lg:flex bg-white rounded-full p-3 text-bodyColor disabled:opacity-30 disabled:cursor-not-allowed transition-opacity shadow-sm border border-gray-200 flex-shrink-0">
      ...
      <div class="w-full max-w-[668px] mx-auto mb-0">
        ...
        <FormHeader v-if="currentStepData" :step-data="currentStepData" />
      </div>
      ...
      <div class="relative pb-32 lg:pb-12 w-full max-w-[668px] mx-auto">
        <FormStepLayout v-if="currentStepData" :step-data="currentStepData" :form-answers="formAnswers"
          :is-loading="isLoading" class="mt-8" @auto-advance="handleAutoAdvance" />
...
// Wrapper around nextStep to fire GTM event when quiz is completed and scroll to top
...
const handleAutoAdvance = async () => {
  ...
  if (isEntireQuizComplete.value) {
    // Fire GTM/GA4 completion events
    if (nuxtApp.$googleTagManager) {
      (nuxtApp.$googleTagManager as any).push({ event: 'QUIZ_FINISH' });
    }
    if (nuxtApp.$ga4) {
      nuxtApp.$ga4.trackLead();
    }
