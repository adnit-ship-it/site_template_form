...
export const useEnvMode = () => {
  ...
  // NMI Collect.js tokenization (public) key. Only consumed by the NMI
  // checkout path (`site_config.use_stripe === false`). Same staging/prod
  // selection rule as the Stripe publishable key above.
  const nmiTokenizationKey = computed(() => {
    return (useStaging.value
      ? cfg.nmiTokenizationKeyStaging
      : cfg.nmiTokenizationKeyProd) as string;
  });
