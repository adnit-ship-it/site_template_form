...
export function useCheckout() {
  // Products catalog (DB-backed) — used to seed the default category when
  // none has been picked yet. Reads from `useState('products-catalog')`,
  // populated once per page load by `plugins/02.products-catalog.ts`.
  const { categories } = useProductsCatalog()
  ...
  const nextStep = (scrollToTopCallback?: () => void) => {
    ...
    if (currentStep.value === 1) {
      ...
      // Everflow: fire `select_product` at the canonical step-1 → step-2
      // transition. Fires regardless of how the user advances (currently
      // FrequencySelector.selectPlanAndAdvance, but any future trigger
      // routes through this same nextStep). The composable's own guards
      // (offer_id + named event must be configured in Supabase) decide
      // whether anything actually goes out to Everflow.
      useEverflow().productSelected({
        product: selectedProduct.value.name,
        plan: selectedPlan.value ?? 'monthly',
      }).catch((err) => {
        console.warn('Everflow productSelected failed (non-critical):', err)
      })
    }
  }
  ...
  const loadStoredData = () => {
    ...
    // If no category selected, fall back to the configured default quiz
    // category (data/common.json → quiz.defaultQuizId, optionally
    // overridden via Supabase `site_config.default_quiz_id_override`),
    // then to the first category in the catalog as a last resort.
    if (!selectedCategory.value) {
      const defaultCategory =
        categories.value.find(p => p.id === getDefaultQuizId()) ||
        categories.value[0]
      if (defaultCategory) {
