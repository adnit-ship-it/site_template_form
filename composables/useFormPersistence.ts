...
export function useFormPersistence(quizId: string) {
  ...
  // `preCalculateOptions` is optional and lets the caller inject the
  // conditional-options pre-calculator. We accept it as a parameter (rather
  // than reaching out to `window.preCalculateConditionalOptions`) so the
  // persistence layer stays free of global side effects.
  const restoreClientState = (
    allSteps: FormStep[],
    formAnswers: FormAnswers,
    currentStepIndex: Ref<number>,
    preCalculateOptions?: (steps: FormStep[], answers: FormAnswers) => void
  ) => {
    if (!process.client) return;
    ...
    if (savedData) {
      ...
      // Pre-calculate conditional options BEFORE step validation so the
      // form doesn't fall back to earlier steps on a double refresh.
      preCalculateOptions?.(allSteps, formAnswers);
