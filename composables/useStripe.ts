...
export const useStripe = () => {
  ...
  const initializeStripe = async () => {
    ...
    try {
      ...
      // Pick the publishable key based on environment mode: staging key
      // when this is a dev build OR when we're running inside an iframe
      // embed (so partners never hit a live Stripe account). See
      // composables/useEnvMode.ts.
      const { stripePublishableKey, useStaging } = useEnvMode();
      ...
      if (useStaging.value) {
        console.info("[useStripe] Using Stripe test key (dev build or embed)");
      }
